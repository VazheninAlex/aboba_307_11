...
int main() {
  ...
  for (int i = 0; i < number; ++i) {
    ...
    root1 = tree1.Insert(root1, std::make_pair(input1, input2));
    root2 = tree2.Insert(root2, std::make_pair(input2, input1));
  }
  int number2;
  std::cin >> number2;
  for (int i = 0; i < number2; ++i) {
    std::cin >> input1;
    if (tree1.Find(root1, input1) == "Errrrrror") {
      std::cout << tree2.Find(root2, input1) << "\n";
    } else {
      std::cout << tree1.Find(root1, input1) << "\n";
    }
  }
  tree1.Clear(root1);
  tree2.Clear(root2);
