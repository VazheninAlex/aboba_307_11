...
class Tree {
 public:
  ...
  std::pair<Node*, Node*> Split(Node* root, long long splitKey) {
    if (root == nullptr) {
      return std::make_pair(nullptr, nullptr);
    }
    if (root->key <= splitKey) {
      std::pair<Node*, Node*> res = Split(root->right_son, splitKey);
    ...
    if (root->key > splitKey) {
      std::pair<Node*, Node*> res = Split(root->left_son, splitKey);
  ...
  Node* Insert(Node* root, long long in) {
    ...
    if (root == nullptr) {
      ...
      std::mt19937 gen(rd());
      Node* temp = new Node(in, gen() % randLimit, 0, nullptr, nullptr);
      return temp;
    }
...
int main() {
  ...
  while (std::cin >> command >> val) {
    if (command == "insert") {
      root = tree.Insert(root, std::stoi(val));
    }
    if (command == "exists") {
      if (tree.Exists(root, std::stoi(val))) {
        std::cout << "true\n";
      } else {
        std::cout << "false\n";
      }
    ...
    if (command == "delete") {
      root = tree.Delete(root, std::stoi(val));
    }
    if (command == "next") {
      long long ans = tree.Next(root, std::stoi(val));
      tree.Ans(ans);
    }
    if (command == "prev") {
      long long ans = tree.Prev(root, std::stoi(val));
      tree.Ans(ans);
    }
    if (command == "kth") {
      long long ans = tree.Kth(root, std::stoi(val));
      tree.Ans(ans);
    }
  }
  tree.Clear(root);
